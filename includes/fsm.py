...
class BettingStatus(State):
    ...
    def __call__(self, common_vars, button_status):
        if common_vars.player_cash_1p >= LOWEST_BET or sum(self._current_bet_1p) > 0:
            plot_chips_1p(
                common_vars.screen, 
            ...
            elif sum(self._current_bet_2p) > 0:
                button_status.play = True
                button_status.undo_bet = True
            else:
            ...
            chips_collide_instance_1p = ChipsCollideArea.get_instance(common_vars)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if len(self._current_bet_1p) < 14:
                        self._chips_visible = True
                        if chips_collide_instance_1p.chip_5_area_1p.collidepoint(mouse_position[0], mouse_position[1]) \
                                and common_vars.player_cash_1p >= 5:
                            # chip_sound.play()
                            self._current_bet_1p.append(5)
                            common_vars.player_cash_1p -= 5
                        elif chips_collide_instance_1p.chip_10_area_1p.collidepoint(mouse_position[0], mouse_position[1]) \
                                and common_vars.player_cash_1p >= 10:
                            # chip_sound.play()
                            self._current_bet_1p.append(10)
                            common_vars.player_cash_1p -= 10
                        elif chips_collide_instance_1p.chip_50_area_1p.collidepoint(mouse_position[0], mouse_position[1]) \
                                and common_vars.player_cash_1p >=50:
                            # chip_sound.play()
                            self._current_bet_1p.append(50)
                            common_vars.player_cash_1p -= 50
                        elif chips_collide_instance_1p.chip_100_area_1p.collidepoint(mouse_position[0], mouse_position[1]) \
                    ...
                    if len(self._current_bet_2p) < 14:
                        self._chips_visible = True
                        if chips_collide_instance_1p.chip_5_area_2p.collidepoint(mouse_position[0], mouse_position[1]) \
                                and common_vars.player_cash_2p >= 5:
                            # chip_sound.play()
                            self._current_bet_2p.append(5)
                            common_vars.player_cash_2p -= 5
                        elif chips_collide_instance_1p.chip_10_area_2p.collidepoint(mouse_position[0], mouse_position[1]) \
                                and common_vars.player_cash_2p >= 10:
                            # chip_sound.play()
                            self._current_bet_2p.append(10)
                            common_vars.player_cash_2p -= 10
                        elif chips_collide_instance_1p.chip_50_area_2p.collidepoint(mouse_position[0], mouse_position[1]) \
                                and common_vars.player_cash_2p >=50:
                            # chip_sound.play()
                            self._current_bet_2p.append(50)
                            common_vars.player_cash_2p -= 50
                        elif chips_collide_instance_1p.chip_100_area_2p.collidepoint(mouse_position[0], mouse_position[1]) \
    ...
    def __call__(self, common_vars, button_status):
        ...
        if len(common_vars.dealer_cards) < 2:
            ...
            for hand in common_vars.player_hands_1p:
                ...
                if value_of_players_hand == 21 and len(common_vars.player_hands_1p) != 2:
                ...
                elif len(common_vars.player_hands_1p) != 2 and is_possible_split(hand):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                value_of_players_hand = get_value_of_players_hand(common_vars.player_hands_1p[0])
                # TODO:
                test = betting(common_vars.player_hands_1p[0], common_vars.dealer_cards[1], value_of_players_hand)
                if button_status.hit and test == "hit":
                    # card_sound.play()
                    card = common_vars.shoe_of_decks.pop()
                    common_vars.player_hands_1p[first_hand].append(card)
                ...
                elif button_status.stand and test == "stand":
...
class PlayerHitState(State):
    ...
    def __call__(self, common_vars, button_status):
        ...
        num_of_hands = len(common_vars.player_hands_1p)
        if num_of_hands == 2:
        ...
        value_of_players_hand = get_value_of_players_hand(common_vars.player_hands_1p[self._current_hand])
...
class DealerInitState(State):
    ...
    def __call__(self, common_vars, button_status):
        ...
        value_of_dealer_hand = get_value_of_dealers_hand(common_vars.dealer_cards)
        common_vars.dealer_last_hand = value_of_dealer_hand
        value_of_player_hand = get_value_of_players_hand(common_vars.player_hands_1p[self._current_hand])
